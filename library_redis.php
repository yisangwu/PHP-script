...
class Lib_Redis{
    ...
    public function __construct( $redisConf=array() ){
        if( !extension_loaded('redis') ){ //检查是否安装了redis扩展
        ...
        if( function_exists('serialize') ){ //PHP支不支持数据的序列化
            $this->_serialize = TRUE;
        }
        if( function_exists('gzcompress') ){ //PHP支不支持数据的压缩
